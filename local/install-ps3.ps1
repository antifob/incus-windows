...
get-wmiobject win32_logicaldisk | % {
	if ($_.volumename -like 'STUFF*') {
		start-process ($_.deviceid+'/local/dotNetFx40_Full_x86_x64.exe') -wait -argumentlist '/q /norestart'
		start-process wusa.exe -wait -argumentlist ($_.deviceid+'/local/Windows6.1-KB2506143-x64.msu /quiet')
	}
}
